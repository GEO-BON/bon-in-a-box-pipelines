...
export function PipelinePage(props) {
  ...
  function showMetadata() {
    if (pipelineMetadata) {
      ...
      return (
        <FoldableOutput title="Pipeline description" isActive={true}>
          <pre key="metadata">
      ...
      >
        ...
        {pipelineMetadata.outputs &&
          Object.entries(pipelineMetadata.outputs).map((entry) => {
            ...
            const value =
              pipelineOutputResults[script] &&
              pipelineOutputResults[script][outputId];

            if (!value) {
              return (
                <div key={outputId} className="outputTitle">
...
function DelayedResult({
      ...
      fetch("output/" + folder + "/output.json")
        ...
        .then((json) => {
          ...
          setPipelineOutputResults((results) => {
            if (id in results) results[id] = json;
  ...
  return (
    <FoldableOutputWithContext
      title={step.replaceAll(">", " > ").replace(/.yml$/, "")}
      componentId={id}
      inline={inline}
      className={className}
    >
