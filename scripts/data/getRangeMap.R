...
packages <- list("rjson", "dplyr", "tidyr", "purrr", "sf", "stringr")
...
# Define species
{
  ...
  for (i in 1:length(sp)) {
    ...
    if (file.exists(file)) {
      ...
      biab_error_stop("========== No range map available for ", sp[i], " at the ", expert_source, " expert source database ==========")
    }
  ...
  # Outputing result -----------------------------------------------------
  biab_output("sf_range_map", v_path_to_range_map)
}
