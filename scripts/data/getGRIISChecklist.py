...
country_name = country_input['country']['englishName']
print(country_name)

if len(country_name.split()) == 2:
    word1, word2 = country_name.split()
    country_name = f"{word1}%20{word2}"

url = f"https://api.checklistbank.org/dataset?offset=0&q=Global%20Introduced%20and%20Invasive%20Species%20-%20{country_name}"
...
if response.status_code == 200:
    ...
    print(results)
    gbif_key = results[0].get("gbifKey")


    # Find dataset(s) where the title contains the country name
    matching = [
        item for item in results
        if country_name.lower() in item.get("title", "").lower()
    ]
    ...
    print(f"Dataset keys for {country_name}:", keys)
    
else:
    print(f"Request failed: {response.status_code}")
    biab_error_stop("Failed to retrieve GRIIS checklist")

biab_output("gbif_key", gbif_key)
