...
get_observations <-
  function(species,
    ...
    year_range_gbif <- year_start
    year_range_atlas <- year_start
    
    if (!is.null(year_end)) {
      year_range_gbif <-
        paste(year_range_gbif, year_end, sep = ",") #string separated by comma
      year_range_atlas <-
        seq(from = year_range_atlas, to = year_end) # sequence of integers
