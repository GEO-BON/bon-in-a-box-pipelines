...
a<- file.path(dirname(input$grid_polygon), "sp_map.GeoJSON")
b<- file.path(dirname(input$grid_polygon), "sp_map.GeoJSON")
...
for (i in 1:2) {
  ...
  layer_path <- file.path(folder_adjustLayers, names(list_layers_adjust)[i] ) %>% paste0(".GeoJSON")
  sf::st_write(sf::st_read(list_layers_adjust[[i]]), layer_path, delete_dsn= T, driver = "GeoJSON")
