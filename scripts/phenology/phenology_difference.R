...
if (num.col > 3) {
    ...
    summary.dat.long <- summary.dat
    print("summary.dat.long:")
    print(summary.dat.long)
}

print(summary.dat.long[3])

if ("band" %in% colnames(summary.dat.long)) {
    # For multiple bands
    ...
    # For one band
    band <- colnames(summary.dat.long)[3]
    phenology_change_plot <- ggplot(summary.dat.long, aes(x = date, y = .data[[band]])) +
        geom_col(fill = "#1d7368") +
        labs(y = "summary", title = paste(band)) +
