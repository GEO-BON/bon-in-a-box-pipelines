...
###rename pdg_table
colnames(pdg_table)<-1:ncol(pdg_table)
rownames(pdg_table)<-unique(SDM$ID)
...
print(CC)
###set y scale
diff<-pdg_table[1]-pdg_table[22]
y<-round_any((max(diff/pdg_table[,1]))*100, 10, f=ceiling)
###Create plot file
file.out<-file.path(outputFolder, "relative_loss1.pdf")## Script body
{
  PchSizes = seq(0.8,2,length.out=4)
  #AreaClasses = c(0, seq(round_any(min(pdg_table),10, f = ceiling),round_any(max(pdg_table),50), length.out=3), 1000)
  ...
  for (pdg in rownames(pdg_table)) {

    area = as.numeric(pdg_table[pdg,])

    perc.lost = (-1+(area[-1]/area[1]))*100

    lines(as.numeric(colnames(pdg_table))[-1], perc.lost, col=adjustcolor(CC[pdg], 0.5), lwd=1)

    points(as.numeric(colnames(pdg_table))[-1], perc.lost, col=adjustcolor(CC[pdg], 0.5), pch=16, cex=PchSizes[cut(area, breaks=AreaClasses)])

  }


  # add lines of overall loss of cells
  area.overall = apply(pdg_table, 2, sum)
  perc.lost = (-1+(area.overall[-1]/area.overall[1]))*100
  lines(as.numeric(colnames(pdg_table))[-1], perc.lost, lwd=3, col=adjustcolor(1,0.5))
  axis(2)
