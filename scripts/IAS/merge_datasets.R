...
griis_clean <- read.csv(input$griis_clean, stringsAsFactors = FALSE)
first_records_clean <- read.csv(input$first_records_clean, stringsAsFactors = FALSE)
FileInfo <- read.csv(input$file_info, stringsAsFactors = FALSE)

datasets_in <- list(
  GRIIS = griis_clean,
  FirstRecords = first_records_clean
)


MergeDatabases <- function(datasets_in = NULL,
                           FileInfo = NULL){
  
  if (is.null(datasets_in) || length(datasets_in) == 0) {
    stop("No cleaned datasets provided.")
  }
  if (is.null(FileInfo)) {
    stop("No FileInfo provided.")
  }
  ...
  for (i in seq_along(datasets_in)){#
    ...
    dat <- datasets_in[[i]]
    dataset_name <- names(datasets_in)[i]
    if (is.null(dataset_name) || is.na(dataset_name) || dataset_name == "") {
      dataset_name <- FileInfo[i, "Dataset_brief_name"]
    }

    cnames <- colnames(dat)
    cnames <- cnames[!cnames%in%c("taxon_orig","location_orig","Country_ISO","ISO3","eventDate_orig","Taxon_group")]
    dat <- dat[,colnames(dat)%in%cnames]
    if ("Kingdom_user" %in% colnames(dat)) {
      colnames(dat)[colnames(dat) == "Kingdom_user"] <- "kingdom"
    }
    if (!"kingdom" %in% colnames(dat)) {
      dat$kingdom <- NA_character_
    }

    dat$origDB <- dataset_name
    ...
    if (i == 1) {
      ...
      alldat <- merge(alldat, dat, by = c("location", "locationID", "taxon", "scientificName", "taxonID", "kingdom", "establishmentMeans"), all = TRUE)
      ...
      alldat <- alldat %>%
        group_by(location, locationID, taxon, scientificName, taxonID, kingdom) %>%
      ...
      alldat <- alldat |>
        mutate(.by = c(location, locationID, taxon, scientificName, taxonID, kingdom),
               g = (establishmentMeans %in% c("introduced", "uncertain") &
                      all(c("introduced", "uncertain") %in% establishmentMeans))) |>
        reframe(.by = c(location, locationID, taxon, scientificName, taxonID, kingdom, g),
  ...
  alldat <- dt[,lapply(.SD,function(s) paste(s,collapse="; ")),by=list(taxon,location,locationID,taxonID,scientificName,kingdom,establishmentMeans)]
  ...
  # clean strings
  ind_col <- which(!colnames(alldat)%in%c("taxon","taxonID","location","locationID","scientificName","kingdom","establishmentMeans"))
  ...
  for (j in seq_len(nrow(all_dat_dupl))){
    ...
    for (k in 1:dim(alldat)[2]){ # loop over columns
      if (colnames(alldat)[k]%in%c("location", "stateProvince","locationID","taxon","scientificName","taxonID","kingdom","establishmentMeans")) next # ignore these columns
  ...
  ## sort output
  oo <- order(alldat$location, alldat$scientificName)
  ...
  key_columns <- c("location", "locationID", "taxon", "scientificName", "taxonID", "kingdom", "eventDate", 
  ...
  columns_out <- columns_out[columns_out %in% colnames(alldat)]
  ...
  # Remove additional quote characters from the reference column when present.
  if ("bibliographicCitation" %in% colnames(alldat_out)) {
...
merged_dataset <- MergeDatabases(datasets_in = datasets_in, FileInfo = FileInfo)

merged_dataset_path <- file.path(outputFolder, "Merged_IAS_dataset.csv")
write.csv(merged_dataset, merged_dataset_path, row.names = FALSE)

biab_output("merged_dataset", merged_dataset_path)
