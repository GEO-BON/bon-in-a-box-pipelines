...
ratio=size_matrix[1]/pop_habitat_area[[1]][1]

NEs<-cbind(Pop_HA_T*size_matrix[,1] ,Pop_HA_T*size_matrix[,2] )
NEs<-rbind(c(round(mean(NEs[,1]>500),2), round(mean(NEs[,2]>500),2)), NEs)
rownames(NEs)[1]<-"NE>500"
ne_table=data.frame(NEs)
ne_table_path = file.path(outputFolder, 'NE.tsv')
write.table(ne_table, ne_table_path, append = F, row.names = F, col.names = F, sep = "\t", quote=F)
ne_table=t(ne_table)
...
rel_area<-pop_habitat_area / pop_habitat_area[, 1]
pop_dyn<-ne_table[,1][2:nrow(ne_table)]*rel_area
...
for (pop in rownames(int_pop_habitat_area)) {
  ...
  for (y in colnames(int_pop_habitat_area)) {
    ...
    NE = min(c(HA,HA_min_past))*ratio[1] # effective population size estiamate, if there is an area increase, uses minimal past area (area decline decrease Ne, but area expansion does not increase Ne)
