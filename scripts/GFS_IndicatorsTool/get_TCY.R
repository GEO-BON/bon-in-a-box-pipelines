...
## load tree cover data from STAC
for(i in c(1,2)){
  if (bbox[i] %% 10 == 0) {
    bbox[i] <- bbox[i] + 0.1
  }
...
load_stac<-function(staccollection, resamplingMethod, noData=NULL){
...
TCL<-load_stac("gfw-lossyear", resamplingMethod = 'mode', noData = 0) # resampling: mode while excluding 0s, find out in which year most of the pixel was lost. 
...
## 2000
TCY = raster::stack(TC>30) # first year: TC when canopy density >30%
names(TCY) = 'y2000'
...
for (y in 1:23) {
  ...
  tcy = (TCY[[paste0('y20',sprintf('%02d', y-1))]]==T)& # forest exist if: (1) forest was there the previous year ...
        (TCL!=y) # ...and (2) forest present did not disappeaer during current year.
  
  # add to stack
  TCY[[paste0('y20',sprintf('%02d', y))]] = tcy
