...
output<- (function(){
  ...
  vector_polygon<-  terra::vect(input$vector_polygon) %>% terra::aggregate()  %>%  terra::project( crs_basemap    )
  ...
  output<- list(vector_polygon = json_study_area_path, vector_grid= raster_study_area_path)
