...
df_SHI_path <- file.path(outputFolder, "df_IUCN_sheet.tsv")
write_tsv(df_IUCN_sheet,file= df_SHI_path)
...
#   
#   sf_area_lim <- st_intersection(sf_area_lim2,sf_area_lim1 %>% st_transform(st_crs(sf_area_lim1)))#NOT SOLVED ERROR
...
# 
# img_SHI_time_period_path <- file.path(outputFolder,sp,"_GFW_change.png")
# tmap_save(img_map_habitat_changes, img_SHI_time_period )
# 
...
# 
# df_SHI_path <- file.path(outputFolder,sp,"_SHI_table.tsv")
...
#                "df_SHI" = df_SHI_path )
output <- list("df_SHI"=df_SHI_path)
# 
