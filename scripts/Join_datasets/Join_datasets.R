...
if(length(new.packages)) {install.packages(new.packages, binary=T)}
...
output<- (function(){
  ...
  ## load data ####
  list_datasets<- lapply(input$lists_datasets, function(x) data.table::fread(x) %>% as.data.frame())
  ...
  write.csv(data_join, data_join_path) # write result
