...
if (type_analysis == "PCA") {
  ...
  pca_vars <- terra::princomp(variables, cor = TRUE, fix_sign = TRUE) # cor true equals scaled
  ...
  pca_importance <- function(x) {
    ...
    vars <- vars / sum(vars)
    rbind(
      `Standard deviation` = round(x$sdev, 2), `Proportion of Variance` = 100 * round(vars, 2),
...
other_args <- list(arguments = list(
...
set.seed(1)
colors_vect <- (sample(
  grDevices::colors()[grep("gr(a|e)y",
    grDevices::colors(),
    invert = T
  )],
...
q <- ggplot2::ggplot() +
  ...
  ggplot2::geom_raster(
    data = merged_df[, c("x", "y", "Block")],
    ggplot2::aes(
      x = x,
      y = y,
      fill = as.factor(Block)
    )
  ) + # make factor if using manual scale
...
ggplot2::ggsave(blocks_plot,
  filename = plot_blocks_map_path,
