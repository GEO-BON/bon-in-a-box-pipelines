...
clean_coordinates <- function(x,
  ...
  if (!is.null(predictors)) {
    ...
    extracted_values <- terra::extract(predictors, dplyr::select(x, all_of(c(lon, lat ))) %>%
                                 data.frame()) 
    
  } else {
    
    extracted_values <- dplyr::select(x, dplyr::all_of(c(covars))) 
    
  }
  
  comp <-  complete.cases(extracted_values)
  x <-  x[comp, ]
  
  if (nrow(x) == 0) stop("All occurrence points are outside the predictor variable rasters")
  message(sprintf("Removed %s records with no environmental data.", nrow(extracted_values) - nrow(x)))
  ...
  if (report) {
    ...
    if (!is.null(species_name)) {
      output_dir <- paste(output_dir, species_name, sep = "/")
    }
    dir.create(file.path(output_dir), showWarnings = FALSE) #dir.create() does not crash if the directory already exists}
    
    report <- paste0(output_dir, "/clean_coordinates_report.csv")
    flag_occ <- paste0(output_dir, "/flagg_occ.csv")
  }
...
cc_env_out <- function(x, 
  ...
  if (!is.null(predictors)) {
  extracted_values <- terra::extract(predictors,
                            dplyr::select(x, all_of(c(lon, lat))))
}

if (is.null(cols)) {
  cols <- names(extracted_values)
}
  nb_var <- floor(threshold * length(cols))
  
  out <- dplyr::select(extracted_values, all_of(cols)
