...
packagesNeed<- c("magrittr", "terra", "raster", "sf", "pbapply", "this.path", "rjson", "auk", "countrycode") # Define the list of required packages to run the script
...
ebd_filters <- ebd %>% auk::auk_species(input$species) %>% # filtrar por especies de interes
  auk::auk_protocol(protocol = input$protocol)  %>% # filtro por portocolo - Metodologia de muestreoo. 
  auk::auk_country(input$country) %>% # Filtro por pais
  auk::auk_bbox(bbox_studyarea) %>%  # Filtrar por extension geografica
  #auk_state(state = "CALDAS") # Filtrar por jurisdiccion
  auk::auk_date(input$date) %>% 
...
dir_ebdfile<- file.path(outputFolder, paste0(paste(c("ebdfile", input$species, round(bbox_studyarea)), collapse = "_"), ".txt")) # Define the file path for the 'val_wkt_path' output
...
pre_ebd_zf <- tryCatch({ 
  auk::auk_zerofill(x= dir_ebdfile, sampling_events =  dir_ebd_sampling_file, collapse = TRUE)
...
dir_ebd_zerofill<- file.path(outputFolder, paste0(paste(c("zf_ebdfile", input$sp, round(bbox_studyarea)), collapse = "_"), ".csv")) # 
...
write.csv(ebird, dir_ebd_zerofill) ## exportar zf
