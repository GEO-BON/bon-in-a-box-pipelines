...
If you already have a consolidated file that includes site and detection variables per sampling event, proceed to [detHistory_occ_model pipeline](). Similarly, if you already have a file in unmarked format for analysis, go to the simple script [occ_model_results]().
...
To facilitate the visualization of the pipeline, download the SVG version of the pipeline available at [Camtrap_occ_model.svg](README_figures/full_workflow.svg), or the HTML version of this documentation available at [Camtrap_occ_model.html](README_figures/full_workflow.svg).
...
* 1.2: Path to the CSV file with sampling data (default: `/inputs/camtrap_sp_covs.csv`).
...
library(magrittr)
data_records<- data.table::fread("C:/Repositories/biab-2.0/scripts/Filter_data/input/records_data.csv") %>% dplyr::filter(scientificName %in% "Cuniculus paca") %>%
dplyr::select(c("scientificName", "eventDate", "eventTime", "Cam.Site"))
knitr::kable(head(data_records),  caption = "Example of input 1.1: Dataset with camera trap record data, including the necessary columns for the analysis.")
```

```{r,  eval= T, echo=F, warning= F, message = F}
library(magrittr)
data_sample<- data.table::fread("C:/Repositories/biab-2.0/scripts/Join_datasets/input/sp_covs.csv")  %>% 
  dplyr::select(c("Cam.Site", "Lat_X", "Long_Y", "Instal.Date", "Last.eventDate"))
knitr::kable(head(data_sample),  caption = "Example of input 1.2: Dataset with camera trap sampling data, including the necessary columns for the analysis.")
```

Inputs 1.3 and 1.4 are used in [FilterData box]() to obtain only the records of the species of interest, while 2.1 and 2.2 will be necessary in [CamTrap_format box]()  as metadata for date and time to organize events in the detection matrix.
...
* 2.5: Name of the observation variables (default: `eventTime|1|hour`). If NULL, it is ignored. Several can be specified separated by commas (ex. `eventTime|1|hour`, `effort|1, moonPhase`). If only the name is specified, the variable collapses according to unique values. If the specified variable column is numeric, it rounds using name|rounding_value, and if it is temporal with name|rounding_value|time_unit. Factors are not rounded. Temporal units can be specified according to all formats detailed in lubridate::round_date. Specification errors will result in them being ignored.
...
* 3.6: Path to the STAC repository where the landscape covariates of interest are located (default: `https://io.biodiversite-quebec.ca/stac/`). If set to NULL, no layers are loaded from the STAC repository.
* 3.7: Name of the collection and layer of the specified STAC repository of the landscape covariates of interest (default: `chelsa-clim|bio1, chelsa-clim|bio2`). Ignored if 3.6 is NULL.
