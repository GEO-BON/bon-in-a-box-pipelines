...
Estos componentes deben estar presentes en la carpeta `~/scripts` del repositorio para que se visualicen en Bon in a Box. Ambos archivos deben estar alojados en la misma carpeta para facilitar su lectura y conexión. Además, se recomienda que en esta carpeta conjunta se incluya ademas una subcarpeta `input` con datos por defecto, lo cual facilitará su prueba y ejecución, sirviendo como guía para los usuarios.
...
* 2. Borrar el caché de carpetas en output: Limpiar los resultados previos para evitar conflictos de interpretación, y mantener solo el folder del intento mas reciente que genera el error para que el código se ejecute correctamente con los datos actuales.
